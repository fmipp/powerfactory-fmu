...
def sim_PFTestTriggers( zip_command, expected_results, fmi_version ):
   ...
   work_dir = os.path.join( os.path.dirname( __file__ ), '..', '..', 'examples', 'triggers' ) # define working directory
...
@pytest.fixture
def fix_PFTestTriggers_expected_results():
   expected_results = {
      0.: 1.03205,
      60.: 1.02631,
      120.: 1.02036,
...
@pytest.mark.fmi1
def test_fmi1_PFTestTriggers( fix_zip_command, fix_PFTestTriggers_expected_results ):
   sim_PFTestTriggers( fix_zip_command, fix_PFTestTriggers_expected_results, 1 )


@pytest.mark.fmi2
def test_fmi2_PFTestTriggers( fix_zip_command, fix_PFTestTriggers_expected_results ):
